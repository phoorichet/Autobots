  ...
  module V1
    class SpeedtestsController < ApplicationController
      ...
      def metric      
        ...
        results = MetricSpeedtest.vtype(vspec[:vtype])
                              .xaxis(vspec[:x])
                              .yaxis(vspec[:y])
                              .aggregate("avg")
                              .stack(vspec[:stack])
      ...
      def mapreduce_join_mslocation
        ...
        select_list = metric.selectfs.map{|d| "#{RawSpeedtest.table_name}.#{d.field}"}
        ...
        results = results.joins("LEFT JOIN #{MsLocation.table_name} ON #{RawSpeedtest.table_name}.imei = #{MsLocation.table_name}.imei")
